...
var shield_bullets: Array[Node2D] = []
...
func _spawn_shield(owner: CharacterBody2D) -> void:
	_remove_shield() # safety: clear old shield first
	for i in range(num_bullets):
		var bullet := shield_bullet_scene.instantiate()
		owner.get_parent().add_child(bullet)

		# Position around fish/player
...
func _process(delta: float) -> void:
	...
	for b in shield_bullets:
		...
		# Orbit logic
		var angle: float = b.get_meta("orbit_angle")
